...
def agent_summarize(summarizer_llm, content: str, kind: str, meta: Dict) -> str:
    ...
    # Safety check: If chunk is empty/None, skip API call
    if not content or not content.strip():
        return ""

    # 1. Prepare context context
    context_str = f"Trial: {meta.get('trial_id')}, Phase: {meta.get('trial_phase')}, Drug: {meta.get('intervention_drug')}"
    
    if kind == "table": 
        instr = "Summarize this clinical table. Focus on data values and trends."
    elif kind == "image": 
        instr = "Describe this clinical figure. Focus on labels, axes, and key takeaways."
    else: 
        instr = "Summarize this clinical text chunk. Capture inclusion criteria, dosing, or safety signals."
    
    # 2. Define prompt with placeholders (NO f-strings for content!)
    prompt = ChatPromptTemplate.from_messages([
        ("system", instr),
...
def main():
    ...
    for trial_dir in trial_dirs:
        ...
        for pdf_dir in sorted([d for d in trial_dir.iterdir() if d.is_dir()]):
            ...
            # Extraction
            with open(md_path, "r") as f: full_text = f.read()
            ...
            meta.update({"source": str(md_path), "trial_id": trial_dir.name, "pdf_stem": pdf_stem})
            
            print(f"       → {meta.get('trial_phase', '?')} | {meta.get('intervention_drug', '?')} | {meta.get('protocol_id', '?')}")
            ...
            for i, chunk in enumerate(chunks):
                ...
                summary = agent_summarize(summarizer_llm, chunk.page_content, "text", meta)
                doc_id = str(uuid.uuid4())
                
                batch_sum.append(Document(page_content=summary, metadata={**chunk.metadata, "doc_id": doc_id}))
            ...
            if images:
                ...
                for img in images:
                    img_meta = meta.copy()
                    img_meta.update({"chunk_type": "image", "filename": img["filename"]})
                    summary = agent_summarize(summarizer_llm, str(img["path"]), "image", img_meta)
                    doc_id = str(uuid.uuid4())
                    
                    img_batch_sums.append(Document(page_content=summary, metadata={**img_meta, "doc_id": doc_id}))
                    img_batch_orgs.append((doc_id, Document(page_content=f"Image: {img['filename']}", metadata=img_meta)))
